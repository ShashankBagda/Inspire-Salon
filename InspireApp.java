...
public class InspireApp extends JFrame implements ActionListener 
{
    ...
    public void actionPerformed(ActionEvent e) 
    {
            ...
            if(!phoneNumber.equals("Enter Phone No") && phoneNumber.matches(phonePattern))
            {
                ...
                else 
                {
                    ...
                    try 
                    {
                        ...
                        writer.write(phone + " - " + randomNumber);
                        writer.close();

                        ProcessBuilder pb = new ProcessBuilder("python", "otp.py");
                        ...
                        while (flag) 
                        {
                            String otpString = JOptionPane.showInputDialog(null, "Enter the OTP:");
                            ...
                            if(otp == randomNumber)
                            {
                                try 
                                {
                                    ...
                                    writer1.write(name + " / " + phone + " / " + mailID);
                                    writer1.newLine();
                                    writer1.close();
                                } 
                                catch (IOException ex) 
        ...
        if (e.getSource() == proceed) 
        {
            ...
            {
                String totalAmount = "total.txt"; // File name or path
                ...
                else if (onlineRadio.isSelected()) 
                {
                    ...
                    if (!value.isEmpty()) {
                        try 
                        {
                            ProcessBuilder pb = new ProcessBuilder("python", "qr_maker.py");
                            pb.redirectErrorStream(true);
                            Process p = pb.start();
                ...
                try {
                    ...
                    try {
                        String bill = new String ("cart.txt");
                        FileWriter fileWriter = new FileWriter(bill); // FileWriter to write to a file
                        BufferedWriter bufferedWriter = new BufferedWriter(fileWriter); // BufferedWriter for efficient writing
                        
                        String dataBill = nameTextArea.getText() + " / " + phoneTextArea.getText() + " / " + mailTextArea.getText() + "\n\n" + cartData;
                        bufferedWriter.write(dataBill); // Writing text to the file
    
                        bufferedWriter.close(); // Close the writer
    
                        System.out.println("Data has been written to " + bill);

                        // Generate Bill
                        ProcessBuilder pb = new ProcessBuilder("python", "bill.py");
                        pb.redirectErrorStream(true);
                        @SuppressWarnings("unused")
                        Process p = pb.start();
